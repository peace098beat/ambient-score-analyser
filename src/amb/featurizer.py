...
def run(in_file):
    ...
    loudness_time = np.linspace(
        0, 0.002 * (loudness_sone.size - 1), loudness_sone.size)
    ...
    plt.hlines(y=np.mean(loudness_sone), xmin=0, xmax=5,
               colors='gray', linestyle='dashed', linewidth=3)
    ...
    plt.hlines(y=np.mean(sharp_acum), xmin=0, xmax=5,
               colors='gray', linestyle='dashed', linewidth=3)
    ...
    plt.hlines(y=np.mean(y_rough), xmin=0, xmax=5,
               colors='gray', linestyle='dashed', linewidth=3)
